...
namespace FIA_Biosum_Manager
{
	...
	public class frmMain : System.Windows.Forms.Form
    {
		...
		public void button_click(string strText)
		{
			...
			if (this.btnOptimizer.Enabled == false) 
			{
                ...
                else if (strText.Trim().ToUpper() == "LOAD GIS DATA")
                {
                   ...
                   if (bSuccess == true)
                   {
                        string strLogFile = frmMain.g_oEnv.strTempDir + "\\biosum_gis_data_debug.txt";
                        int intRowCount = oGisTools.LoadSqliteGisData(bUpdatePlotYardingDist, strLogFile);
                       if (intRowCount < 1)
                       {
        ...
        public void button_click_sqlite(string strText)
        {
            ...
            if (this.btnOptimizer.Enabled == false)
            {
                ...
                else if (strText.Trim().ToUpper() == "LOAD GIS DATA")
                {
                    ...
                    if (bSuccess == true)
                    {
                        int intRowCount = oGisTools.LoadSqliteGisData(false);
                        if (intRowCount < 1)
                        {
                            MessageBox.Show("An error occurred while loading the GIS data!!", "FIA BioSum");
                            this.DeactivateStandByAnimation();
