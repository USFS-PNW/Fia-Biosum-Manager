	...
	public class RunOptimizer
	{
		...
		public RunOptimizer(FIA_Biosum_Manager.uc_optimizer_scenario_run p_form)
		{
			...
			try
			{
                ...
                // temp accdb and links are created here. might be where to add creation of source db list? maybe not because of gettables
                ...
                this.m_oVarSub.ReferenceGeneralMacroSubstitutionVariableCollection =
                ...
                this.m_strFVSPrePostValidComboDbPathAndFile = frmMain.g_oUtils.getRandomFile(frmMain.g_oEnv.strTempDir, "db");
                this.CopyScenarioResultsTableSqlite(this.m_strFVSPrePostValidComboDbPathAndFile, ReferenceUserControlScenarioRun.ReferenceOptimizerScenarioForm.uc_scenario1.txtScenarioPath.Text.Trim() + "\\db\\validcombo.db");
				...
				else
				{
                    ...
                    //// Manually link to rx and rxpackage tables
                    //if (odbcMgr.CurrentUserDSNKeyExist(ODBCMgr.DSN_KEYS.MasterDsnName))
                    //{
                    //    odbcMgr.RemoveUserDSN(ODBCMgr.DSN_KEYS.MasterDsnName);
                    //}
                    //odbcMgr.CreateUserSQLiteDSN(ODBCMgr.DSN_KEYS.MasterDsnName,m_strRxPathAndFile);

                    //m_oDao.CreateSQLiteTableLink(m_strTempMDBFile, m_strRxTable, m_strRxTable,
                    //    ODBCMgr.DSN_KEYS.MasterDsnName, m_strRxPathAndFile);
                    //m_oDao.CreateSQLiteTableLink(m_strTempMDBFile, m_strRxPackageTable, m_strRxPackageTable,
                    //    ODBCMgr.DSN_KEYS.MasterDsnName, m_strRxPathAndFile);

                    //if (m_oDao.m_intError != 0)
                    //{
                    //    m_oDao.m_strError = "!!Error Creating Treatment Prescription Table Links!!!";
                    //    this.m_intError = m_oDao.m_intError;
                    //}
     ...
     //               }
        ...
        private void CreateTableStructureOfUserDefinedPlotSQL_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            string strConn = p_dataMgr.GetConnectionString(this.m_strWorkTablesDb);
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                conn.Open();
                ...
                using (OleDbConnection tempConn = new OleDbConnection(p_ado.getMDBConnString(this.m_strTempMDBFile, "", "")))
                {
                    ...
                    p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, "userdefinedplotfilter_work", "userdefinedplotfilter_work", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                p_dataMgr.m_strSQL = "ATTACH DATABASE '" + this.m_strWorkTablesDb + "' AS work_tables";
                p_dataMgr.SqlNonQuery(conn, p_dataMgr.m_strSQL);
        ...
        /// </summary>
        private void CopyScenarioResultsTableAccess(string p_strDestPathAndDbFileName,string p_strSourcePathAndDbFileName)
		{
		...
		/// create links to the tables located in the optimizer_results.accdb file
		/// </summary>
		private void CreateScenarioResultTableLinks_access()
		{
        ...
        /// </summary>
        private void CreateValidComboTables_access()
        {
        ...
        /// create links to the tables located in the optimizer_results.accdb file
        /// </summary>
        private void CreateValidComboTableLinks_access()
        {
        ...
        private void CreateValidComboTableLinks()
        {
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(p_dataMgr.GetConnectionString(this.m_strFVSPrePostValidComboDbPathAndFile)))
            {
                conn.Open();
                strTableNames = p_dataMgr.getTableNames(conn);
                intCount = strTableNames.Length;
            ...
            if (p_dataMgr.m_intError == 0)
            {
                ...
                {
                    ODBCMgr odbcmgr = new ODBCMgr();
                    if (odbcmgr.CurrentUserDSNKeyExist(ODBCMgr.DSN_KEYS.PrePostValidComboDsnName))
                    {
                        odbcmgr.RemoveUserDSN(ODBCMgr.DSN_KEYS.PrePostValidComboDsnName);
                    }
                    odbcmgr.CreateUserSQLiteDSN(ODBCMgr.DSN_KEYS.PrePostValidComboDsnName, this.m_strFVSPrePostValidComboDbPathAndFile);

                    for (int x = 0; x <= intCount - 1; x++)
                    {
                        p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, strTableNames[x], strTableNames[x], ODBCMgr.DSN_KEYS.PrePostValidComboDsnName, this.m_strFVSPrePostValidComboDbPathAndFile);
                        if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
                        {
                            frmMain.g_oUtils.WriteText(m_strDebugFile,
                                        "validcombo\t" + strTableNames[x] + "\r\n");
        ...
        private void CreateProcessorScenarioResultTableLinks_access()
        {
            dao_data_access oDao = new dao_data_access();
        ...
        private void CreateProcessorScenarioResultTableLinks()
        {
            ...
            if (oDao.m_intError != 0)
            {
                oDao.m_strError = Tables.ProcessorScenarioRun.DefaultHarvestCostsTableName + " !!Error Creating Table Link!!!";
            ...
            if (oDao.m_intError != 0)
            {
                oDao.m_strError = Tables.ProcessorScenarioRun.DefaultTreeVolValSpeciesDiamGroupsTableName + " !!Error Creating Table Link!!!";
                this.m_intError = oDao.m_intError;
            }
        ...
        /// </summary>
        private void CreateAuditTableLinks_access()
		{

            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
                frmMain.g_oUtils.WriteText(m_strDebugFile, "//CreateAuditTableLinks_access\r\n");
                frmMain.g_oUtils.WriteText(m_strDebugFile, "//\r\n");
            }
			string[] strTableNames;
            int y;
		...
		/// </summary>
		private void CreateScenarioTableLinks_access()
		{
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
		...
		private void FilterPSites_access()
		{
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
        ...
        /// </summary>
        private void CondAccessible_access()
		{
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
                frmMain.g_oUtils.WriteText(m_strDebugFile, "//CondAccessible_access\r\n");
                frmMain.g_oUtils.WriteText(m_strDebugFile, "//\r\n");
            }
        ...
        private void CondAccessible_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(p_dataMgr.GetConnectionString(this.m_strWorkTablesDb)))
            {
                conn.Open();
			    ...
			    *************************************************************/
                p_dataMgr.m_strSQL = "UPDATE " + Tables.OptimizerScenarioResults.DefaultScenarioResultsPSiteAccessibleWorkTableName + 
                    ...
                    "WHERE cond_too_far_steep_yn IS NULL OR (cond_too_far_steep_yn <> 'Y' AND " +
                    "cond_too_far_steep_yn <> 'N');";
                if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                    frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + p_dataMgr.m_strSQL + "\r\n");
                p_dataMgr.SqlNonQuery(conn, p_dataMgr.m_strSQL);
        ...
        private void sumTreeVolVal()
        {
            ...
            using (System.Data.SQLite.SQLiteConnection workConn = new System.Data.SQLite.SQLiteConnection(m_strWorkTablesConn))
            {
                ...
                p_dataMgr.m_strSQL = "INSERT INTO " + m_strTreeVolValSumTable +
                    ...
                    "s.place_holder " +
                    "FROM " + this.m_strTreeVolValBySpcDiamGroupsTable.Trim() + " AS s " + 
        ...
        private void validcombos_old()
        {
            ...
            string strAuditDb = ReferenceUserControlScenarioRun.ReferenceOptimizerScenarioForm.uc_scenario1.txtScenarioPath.Text.Trim() + "\\db\\" + Tables.Audit.DefaultCondAuditTableSqliteDbFile;
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(m_dataMgr.GetConnectionString(strAuditDb)))
            {
                conn.Open();
            ...
            string strRxConn = m_dataMgr.GetConnectionString(this.m_strRxPathAndFile);
            using (System.Data.SQLite.SQLiteConnection rxConn = new System.Data.SQLite.SQLiteConnection(strRxConn))
            {
                rxConn.Open();
                m_dataMgr.m_strSQL = "SELECT rx FROM " + this.m_strRxTable;
                m_dataMgr.SqlQueryReader(rxConn, m_dataMgr.m_strSQL);
                ...
                {
                    strRxList += m_dataMgr.m_DataReader["rx"].ToString().Trim();
                }
            ...
            string strConn = m_dataMgr.GetConnectionString(this.m_strSystemResultsDbPathAndFile);
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                conn.Open();
                this.m_strSQL = "ATTACH DATABASE '" + this.m_strWorkTablesDb + "' AS work_tables";
                if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                    frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                m_dataMgr.SqlNonQuery(conn, this.m_strSQL);

                this.m_strSQL = "DELETE FROM ruledefinitionscondfilter AS a " +
                "WHERE NOT EXISTS " +
           ...
           "WHERE b.simyear4_rx IS NOT NULL AND LEN(TRIM(b.simyear4_rx)) > 0 AND b.simyear4_rx<>'000'";
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                m_strSQL = "ATTACH DATABASE '" + frmMain.g_oFrmMain.frmProject.uc_project1.txtRootDirectory.Text.Trim() + "\\" + Tables.OptimizerScenarioResults.DefaultCalculatedPrePostFVSVariableTableSqliteDbFile + "' AS calc_prepost_fvsout";
                m_dataMgr.SqlNonQuery(conn, m_strSQL);
                //attach fvs_prepost.db
                m_strSQL = "ATTACH DATABASE '" + frmMain.g_oFrmMain.frmProject.uc_project1.txtRootDirectory.Text.Trim() + Tables.FVS.DefaultFVSOutPrePostDbFile + "' AS prepost_fvsout";
                m_dataMgr.SqlNonQuery(conn, m_strSQL);
                //attach fvs_prepost.db
                m_strSQL = "ATTACH DATABASE '" + m_strFVSPrePostValidComboDbPathAndFile + "' AS valid_combo";
                m_dataMgr.SqlNonQuery(conn, m_strSQL);
                ...
                for (x = 0; x <= FIA_Biosum_Manager.uc_optimizer_scenario_fvs_prepost_variables_effective.NUMBER_OF_VARIABLES - 1; x++)
                {
                    ...
                    if (strTable.Trim().Length > 0)
                    {
                        ...
                        if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                            frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                        m_dataMgr.SqlNonQuery(conn, m_strSQL);
                        m_strSQL = "UPDATE validcombos_fvspost SET variable" + Convert.ToString(x + 1).Trim() + "_yn='N' WHERE LENGTH(TRIM(variable" + Convert.ToString(x + 1).Trim() + "_yn))=0";
                        m_dataMgr.SqlNonQuery(conn, m_strSQL);
                    ...
                    if (strTable.Trim().Length > 0)
                    {
                        ...
                        if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                            frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                        m_dataMgr.SqlNonQuery(conn, m_strSQL);
                        m_strSQL = "UPDATE validcombos_fvspre SET variable" + Convert.ToString(x + 1).Trim() + "_yn='N' WHERE LENGTH(TRIM(variable" + Convert.ToString(x + 1).Trim() + "_yn))=0";
                        m_dataMgr.SqlNonQuery(conn, m_strSQL);
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                this.m_strSQL = "INSERT INTO validcombos (biosum_cond_id,rxpackage,rx,rxcycle) " +
                    ...
                    "JOIN " + this.m_strHvstCostsTable + " AS h ON v.rxpackage = h.rxpackage " +
                ...
                this.m_strSQL = "DELETE " +
                    "FROM validcombos as v WHERE " +
                ...
                this.m_strSQL = "DELETE FROM validcombos as v " +
                                "WHERE EXISTS " +
                                 "(SELECT p.* " +
            ...
            if ((bool)frmMain.g_oDelegate.GetControlPropertyValue((System.Windows.Forms.Control)oCheckBox, "Checked", false) == true)
            {
                ...
                _uc_scenario_run.uc_filesize_monitor4.BeginMonitoringFile(strAuditDb, 2000000000, "2GB");
                ...
                using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(m_dataMgr.GetConnectionString(strAuditDb)))
                {
                    ...
                    if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                        frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                    m_dataMgr.SqlNonQuery(conn, this.m_strSQL);
                    this.m_strSQL = "ATTACH DATABASE '" + this.m_strSystemResultsDbPathAndFile + "' AS results";
                    if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                        frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                    m_dataMgr.SqlNonQuery(conn, this.m_strSQL);
                    this.m_strSQL = "ATTACH DATABASE '" + this.m_strProcessorResultsPathAndFile + "' AS processor_results";
                    if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                        frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                    m_dataMgr.SqlNonQuery(conn, this.m_strSQL);
                    this.m_strSQL = "ATTACH DATABASE '" + this.m_strRxPathAndFile + "' AS prescriptions";
                    if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                        frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + this.m_strSQL + "\r\n");
                    m_dataMgr.SqlNonQuery(conn, this.m_strSQL);
                     ...
                     ************************************************************************/
                    this.m_strSQL = "SELECT COUNT(*) FROM " +
                    ...
                    if ((int)m_dataMgr.getRecordCount(conn, this.m_strSQL, "temp") > 0)
                    {
                    ...
                    this.m_strSQL = "UPDATE " + Tables.Audit.DefaultCondAuditTableName + " as a " +
                        ...
                        "WHERE a.harvest_costs_yn IS NULL OR LENGTH(TRIM(a.harvest_costs_yn))=0;";
                    if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
                    ...
                    if ((int)m_dataMgr.getRecordCount(conn, m_strSQL, "temp") > 0)
                    {
                        this.m_strSQL = "UPDATE " + Tables.Audit.DefaultCondAuditTableName + " as a " +
                    ...
                    m_strSQL = "SELECT COUNT(*) FROM " +
                                        "(SELECT a.biosum_cond_id " +
                                        "FROM " + Tables.Audit.DefaultCondAuditTableName + " AS a," +
                                            this.m_strTreeVolValSumTable + " AS b " +
                                        "WHERE a.biosum_cond_id = b.biosum_cond_id LIMIT 1)";
                    if ((int)m_dataMgr.getRecordCount(conn, this.m_strSQL, "temp") > 0)
                    {
                        this.m_strSQL = "UPDATE " + Tables.Audit.DefaultCondAuditTableName + " AS a " +
                    ...
                    m_dataMgr.m_strSQL = "ATTACH DATABASE '" + strGISDbPathAndFile + "' AS gis";
                    m_dataMgr.SqlNonQuery(conn, m_dataMgr.m_strSQL);

                    this.m_strSQL = "UPDATE " + Tables.Audit.DefaultCondAuditTableName +
                    ...
                    m_dataMgr.m_strSQL = "SELECT COUNT(*) FROM " +
                                    "(SELECT a.biosum_cond_id " +
                                    "FROM " + Tables.Audit.DefaultCondAuditTableName + " AS a," +
                    ...
                    if ((int)m_dataMgr.getRecordCount(conn, m_dataMgr.m_strSQL, "temp") > 0)
                    {
                        m_dataMgr.m_strSQL = "UPDATE " + Tables.Audit.DefaultCondAuditTableName +
                    ...
                    this.m_strSQL = $@"UPDATE {Tables.Audit.DefaultCondAuditTableName} AS a SET cond_too_far_steep_yn = p.cond_too_far_steep_yn,
                        psite_merch_yn = CASE WHEN p.merch_haul_psite IS NULL THEN 'N' ELSE 'Y' END, psite_chip_yn = CASE WHEN p.chip_haul_psite IS NULL THEN 'N' ELSE 'Y' END
                        FROM {Tables.OptimizerScenarioResults.DefaultScenarioResultsPSiteAccessibleWorkTableName} AS p WHERE a.biosum_cond_id = p.biosum_cond_id";
                    ...
                    *********************************************************************************/
                    ...
                    m_strSQL = "UPDATE " + Tables.Audit.DefaultCondRxAuditTableName +
                        " SET fvs_prepost_variables_yn = 'Y' " +
                    ...
                    if ((int)m_dataMgr.getRecordCount(conn, this.m_strSQL, "temp") > 0)
                    {
                        this.m_strSQL = "UPDATE " + Tables.Audit.DefaultCondRxAuditTableName + " AS a " +
                            "SET processor_tree_vol_val_yn = 'Y' " +
        ...
        private void Optimization_access()
		{
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
        ...
        private void tiebreaker_access()
		{
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
        ...
        /// treatment on a plot 
        /// </summary>
        private void econ_by_rx_cycle_access()
		{
		...
		private void econ_by_rx_cycle()
        {
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                if (p_dataMgr.TableExist(conn, this.m_strEconByRxWorkTableName))
                {
                    p_dataMgr.SqlNonQuery(conn, "DROP TABLE " + this.m_strEconByRxWorkTableName);
                }

                // Create ecob_by_rx_cycle_work_table in temp work tables db
                // Hve to parse by column source

                string strSelectSQL = "SELECT validcombos.biosum_cond_id,validcombos.rxpackage,validcombos.rx," +
                       "validcombos.rxcycle," +
                       this.m_strTreeVolValSumTable.Trim() + ".merch_vol_cf," +
                       this.m_strTreeVolValSumTable.Trim() + ".chip_vol_cf," +
                       this.m_strTreeVolValSumTable.Trim() + ".chip_wt_gt," +
                ...
                foreach (string column in columnsFromValidCombos)
                {
                    field = "";
                    dataType = "";
                    p_dataMgr.getFieldNamesAndDataTypes(conn, "SELECT " + column + " FROM validcombos", ref field, ref dataType);
                    dataType = utils.DataTypeConvert(dataType.ToUpper(), true);
                ...
                foreach (string column in columnsFromTreeVolValSum)
                {
                    field = "";
                    dataType = "";
                    p_dataMgr.getFieldNamesAndDataTypes(conn, "SELECT " + column + " FROM " + this.m_strTreeVolValSumTable.Trim(), ref field, ref dataType);
                    dataType = utils.DataTypeConvert(dataType.ToUpper(), true);
                    fieldsAndDataTypes += field + " " + dataType + ", ";
                }
                ...
                field = "";
                dataType = "";
                this.m_ado.getFieldNamesAndDataTypes(this.m_TempMDBFileConn, "SELECT complete_cpa FROM " + this.m_strHvstCostsTable.Trim(), ref field, ref dataType);
                dataType = utils.DataTypeConvert(dataType.ToUpper(), true);
                ...
                foreach (string column in columnsFromCondTable)
                {
                    field = "";
                    dataType = "";
                    this.m_ado.getFieldNamesAndDataTypes(this.m_TempMDBFileConn, "SELECT " + column + " FROM " + this.m_strCondTable.Trim(), ref field, ref dataType);
                    dataType = utils.DataTypeConvert(dataType.ToUpper(), true);
                    fieldsAndDataTypes += field + " " + dataType + ", ";
                }
                ...
                p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, this.m_strEconByRxWorkTableName, this.m_strEconByRxWorkTableName, ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                ...
                do
                {
                    ...
                    if (i > 20)
                    {
                        System.Windows.Forms.MessageBox.Show("An error occurred while trying to attach" + this.m_strEconByRxWorkTableName + "table! " +
                        "Validate the contents of this database before trying to run Treatment Optimizer.", "FIA Biosum");
                        break;
                    }
                    System.Threading.Thread.Sleep(1000);
                ...
                while (!this.m_ado.TableExist(this.m_TempMDBFileConn, this.m_strEconByRxWorkTableName));
                ...
                p_dataMgr.m_strSQL = "UPDATE " + this.m_strEconByRxWorkTableName +
                    ...
                    Tables.OptimizerScenarioResults.DefaultScenarioResultsPSiteAccessibleWorkTableName + ".CHIP_HAUL_COST_DPGT THEN 'N' ELSE 'Y' END" +
                    " FROM " + Tables.OptimizerScenarioResults.DefaultScenarioResultsPSiteAccessibleWorkTableName +
                    ...
                    " AND usebiomass_yn = 'Y' AND rxcycle = '1'";
                if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                    frmMain.g_oUtils.WriteText(m_strDebugFile, "Execute SQL: " + p_dataMgr.m_strSQL + "\r\n");
                p_dataMgr.SqlNonQuery(conn, p_dataMgr.m_strSQL);
                if (p_dataMgr.m_intError != 0)
                ...
                string strInactiveStandsWorkTable = "InactiveStandsWorkTable";
                strSelectSQL = "SELECT validcombos.biosum_cond_id,validcombos.rxpackage,validcombos.rx,validcombos.rxcycle," +
                            " 0 AS merch_vol_cf, 0 as chip_vol_cf, 0 as chip_wt_gt," +
                            " 0 as chip_val_dpa, 0 as merch_wt_gt, 0 AS merch_val_dpa," +
                            " harvest_costs.complete_cpa AS harvest_onsite_cost_dpa, 0 AS escalator_merch_haul_cpa_pt," +
                            " 0 AS merch_haul_cost_dpa, 0 AS escalator_chip_haul_cpa_pt, 0 AS chip_haul_cost_dpa," +
                            " 0 AS merch_chip_nr_dpa, 0 AS merch_nr_dpa, 0 AS max_nr_dpa," +
                            " cond.acres, cond.owngrpcd, 0 AS haul_costs_dpa FROM" +
                            " ((validcombos INNER JOIN cond ON validcombos.biosum_cond_id = cond.biosum_cond_id)" +
                            " INNER JOIN harvest_costs ON (validcombos.biosum_cond_id=harvest_costs.biosum_cond_id) AND" +
                            " (validcombos.rxpackage=harvest_costs.rxpackage) AND" +
                            " (validcombos.rx=harvest_costs.rx) AND (validcombos.rxcycle=harvest_costs.rxcycle))" +
                ...
                p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, strInactiveStandsWorkTable, strInactiveStandsWorkTable, ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                ...
                do
                {
                    ...
                    if (i > 20)
                    {
                        System.Windows.Forms.MessageBox.Show("An error occurred while trying to attach " + strInactiveStandsWorkTable + " table! " +
                        "Validate the contents of this database before trying to run Treatment Optimizer.", "FIA Biosum");
                        break;
                    }
                    System.Threading.Thread.Sleep(1000);
                ...
                while (!this.m_ado.TableExist(this.m_TempMDBFileConn, strInactiveStandsWorkTable));

                // still through ado since selectSQL references cond table
                this.m_strSQL = "INSERT INTO " + strInactiveStandsWorkTable + " " + strSelectSQL;
                this.m_ado.SqlNonQuery(this.m_TempMDBFileConn, this.m_strSQL);
                ...
                p_dataMgr.m_strSQL = "INSERT INTO " + this.m_strEconByRxWorkTableName +
                            " SELECT " + strInactiveStandsWorkTable + ".biosum_cond_id," + strInactiveStandsWorkTable + ".rxPackage, " +
                            strInactiveStandsWorkTable + ".rx, " + strInactiveStandsWorkTable + ".rxcycle," +
                            strInactiveStandsWorkTable + ".merch_vol_cf, " + strInactiveStandsWorkTable + ".chip_vol_cf," +
                            strInactiveStandsWorkTable + ".chip_wt_gt, " + strInactiveStandsWorkTable + ".chip_val_dpa," +
        ...
        /// treatment on a plot 
        /// </summary>
        private void econ_by_rx_utilized_sum_access()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
        ...
        private void econ_by_rx_utilized_sum()
        {
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(p_dataMgr.GetConnectionString(this.m_strWorkTablesDb)))
            {
                ...
                p_dataMgr.m_strSQL = "INSERT INTO " + Tables.OptimizerScenarioResults.DefaultScenarioResultsEconByRxUtilSumTableName +
                    ...
                    "FROM " + this.m_strEconByRxWorkTableName + " GROUP BY biosum_cond_id, rxpackage, acres, owngrpcd) AS a";
                ...
                // Create worktable for HVST_TYPE_BY_CYCLE field
                string strTreeSumWorktableName = "ECON_SUM_WORKTABLE";
                if (p_dataMgr.TableExist(conn, strTreeSumWorktableName))
                {
                    p_dataMgr.SqlNonQuery(conn, "DELETE FROM " + strTreeSumWorktableName);
                    if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
                        frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\nDelete all records from econ_sum_worktable\r\n");
                ...
                p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, strTreeSumWorktableName, strTreeSumWorktableName, ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                ...
                do
                {
                    ...
                    if (i > 20)
                    {
                        System.Windows.Forms.MessageBox.Show("An error occurred while trying to attach table! " +
                        "Validate the contents of this database before trying to run Treatment Optimizer.", "FIA Biosum");
                        break;
                    }
                    System.Threading.Thread.Sleep(1000);
                ...
                while (!this.m_ado.TableExist(this.m_TempMDBFileConn, strTreeSumWorktableName));
        ...
        /// </summary>
        private void CreateTableStructureOfHarvestCosts_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            if (p_ado.m_intError == 0)
            {
                ...
                strConn = p_dataMgr.GetConnectionString(this.m_strWorkTablesDb);
                using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
                {
                    conn.Open();
                    ...
                    p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, "harvest_costs_sum", "harvest_costs_sum", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
        ...
        private void CreateTableStructureForScenarioProcessingSites_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            ***********************************************************************/
            ...
            {
                conn.Open();
                if (p_dataMgr.TableExist(conn, this.m_strPSiteWorkTable))
                {
                ...
                p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, this.m_strPSiteWorkTable, this.m_strPSiteWorkTable, ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
        ...
        private void CreateTableStructureForHaulCosts_olc()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            if (oAdo.m_intError == 0)
            {
                ...
                using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(oDataMgr.GetConnectionString(this.m_strWorkTablesDb)))
                {
                    conn.Open();
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, Tables.OptimizerScenarioResults.DefaultScenarioResultsPSiteAccessibleWorkTableName, Tables.OptimizerScenarioResults.DefaultScenarioResultsPSiteAccessibleWorkTableName,
                        ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "all_road_chip_haul_costs_work_table", "all_road_chip_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "all_road_merch_haul_costs_work_table", "all_road_merch_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cheapest_road_merch_haul_costs_work_table", "cheapest_road_merch_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cheapest_road_chip_haul_costs_work_table", "cheapest_road_chip_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cheapest_rail_merch_haul_costs_work_table", "cheapest_rail_merch_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cheapest_rail_chip_haul_costs_work_table", "cheapest_rail_chip_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cheapest_merch_haul_costs_work_table", "cheapest_merch_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cheapest_chip_haul_costs_work_table", "cheapest_chip_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "chip_plot_to_rh_to_collector_haul_costs_work_table", "chip_plot_to_rh_to_collector_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "merch_plot_to_rh_to_collector_haul_costs_work_table", "merch_plot_to_rh_to_collector_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "combine_chip_rail_road_haul_costs_work_table", "combine_chip_rail_road_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "combine_merch_rail_road_haul_costs_work_table", "combine_merch_rail_road_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "merch_rh_to_collector_haul_costs_work_table", "merch_rh_to_collector_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "chip_rh_to_collector_haul_costs_work_table", "chip_rh_to_collector_haul_costs_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
        ...
        /// </summary>
        private void CreateTableStructureForIntensity_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(oDataMgr.GetConnectionString(this.m_strWorkTablesDb)))
            {
                ...
                if (oDataMgr.m_intError == 0)
                {
				    ...
				    *****************************************************************/
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "rx_intensity_duplicates_work_table", "rx_intensity_duplicates_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    int i = 0;
                    do
                    {
                        // break out of loop if it runs too long
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "rx_intensity_duplicates_work_table2", "rx_intensity_duplicates_work_table2", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    i = 0;
                    do
                    {
                        // break out of loop if it runs too long
                    ...
                    oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "rx_intensity_unique_work_table", "rx_intensity_unique_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    i = 0;
                    do
                    {
                        // break out of loop if it runs too long
        ...
        /// </summary>
        private void CreateTableStructureForPlotCondAccessiblity_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                using (OleDbConnection tempConn = new OleDbConnection(p_ado.getMDBConnString(this.m_strTempMDBFile, "", "")))
                {
                    ...
                    p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, "plot_cond_accessible_work_table", "plot_cond_accessible_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                    ...
                    p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, "plot_cond_accessible_work_table2", "plot_cond_accessible_work_table2", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
        ...
        private void CreateTableStructureForUserDefinedConditionTable_old()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                using (OleDbConnection tempConn = new OleDbConnection(p_ado.getMDBConnString(this.m_strTempMDBFile, "", "")))
                {
                    ...
                    p_dao.CreateSQLiteTableLink(this.m_strTempMDBFile, "userdefinedcondfilter_work", "userdefinedcondfilter_work", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(strConn))
            {
                ...
                using (OleDbConnection tempConn = new OleDbConnection(p_ado.getMDBConnString(this.m_strTempMDBFile, "", "")))
                {
                    ...
                    string[] arrFields = p_ado.getFieldNamesArray(tempConn, "SELECT * FROM " + this.m_strCondTable);
                    ...
                    foreach (string column in arrFields)
                    {
                        string field = "";
                        string dataType = "";
                        p_ado.getFieldNamesAndDataTypes(tempConn, "SELECT " + column + " FROM " + this.m_strCondTable, ref field, ref dataType);
                        dataType = utils.DataTypeConvert(dataType.ToUpper(), true);
                        fieldsAndDataTypes = fieldsAndDataTypes + field + " " + dataType + ", ";
                    }
		...
		/// </summary>
		private void Best_rx_summary_access()
		{
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
                frmMain.g_oUtils.WriteText(m_strDebugFile, "//Best_rx_summary_access\r\n");
                frmMain.g_oUtils.WriteText(m_strDebugFile, "//\r\n");
            }
			string strTable="";
			...
			if (this.m_oOptimizationVariable.strOptimizedVariable.Trim().ToUpper() == "REVENUE")
			{
				Best_rx_summary_access(oTieBreakerCollection,strTieBreakerAggregate,false);
			...
			{
				Best_rx_summary_access(oTieBreakerCollection,strTieBreakerAggregate,true);
			}
        ...
        private void Best_rx_summary()
        {
            ...
            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(p_dataMgr.GetConnectionString(this.m_strWorkTablesDb)))
            {
                ...
                oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, strTable, strTable, ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                int i = 0;
                do
                {
                    // break out of loop if it runs too long
                ...
                oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cycle1_best_rx_summary_optimization_and_tiebreaker_work_table", "cycle1_best_rx_summary_optimization_and_tiebreaker_work_table", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                i = 0;
                do
                {
                    // break out of loop if it runs too long
                ...
                oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cycle1_best_rx_summary_optimization_and_tiebreaker_work_table2", "cycle1_best_rx_summary_optimization_and_tiebreaker_work_table2", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                i = 0;
                do
                {
                    // break out of loop if it runs too long
                ...
                oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cycle1_best_rx_summary_optimization_and_tiebreaker_work_table3", "cycle1_best_rx_summary_optimization_and_tiebreaker_work_table3", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                i = 0;
                do
                {
                    // break out of loop if it runs too long
                ...
                oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, strWorkTable, strWorkTable, ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                i = 0;
                do
                {
                    // break out of loop if it runs too long
                ...
                oDao.CreateSQLiteTableLink(this.m_strTempMDBFile, "cycle1_effective_optimization_treatments", "cycle1_effective_optimization_treatments", ODBCMgr.DSN_KEYS.WorkTablesDsnName, this.m_strWorkTablesDb);
                i = 0;
                do
                {
                    // break out of loop if it runs too long
        ...
        /// </summary>
        private void CondPsiteTableAccess()
        {
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
            {
                frmMain.g_oUtils.WriteText(m_strDebugFile, "\r\n//\r\n");
        ...
        private void CondPsiteTable()
        {
            ...
            DataMgr m_dataMgr = new DataMgr();

            using (System.Data.SQLite.SQLiteConnection conn = new System.Data.SQLite.SQLiteConnection(m_dataMgr.GetConnectionString(this.m_strSystemResultsDbPathAndFile)))
            {
                conn.Open();
        ...
        private void ContextReferenceTables()
        {
            ...
            if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 1)
                ...
                p_dataMgr.m_strSQL += " SELECT VARIABLE_NAME, VARIABLE_DESCRIPTION, BASELINE_RXPACKAGE, VARIABLE_SOURCE," +
                                 " weight_1_pre, weight_1_post, weight_2_pre, weight_2_post, weight_3_pre, weight_3_post, weight_4_pre, weight_4_post";
                p_dataMgr.m_strSQL += " FROM " + Tables.OptimizerDefinitions.DefaultCalculatedOptimizerVariablesTableName +
                                 " JOIN " + Tables.OptimizerDefinitions.DefaultCalculatedFVSVariablesTableName + " ON " +
                ...
                p_dataMgr.m_strSQL += " SELECT VARIABLE_NAME, VARIABLE_DESCRIPTION, VARIABLE_SOURCE," +
                                 " weight as CYCLE_1_WEIGHT";
                p_dataMgr.m_strSQL += " FROM " + Tables.OptimizerDefinitions.DefaultCalculatedOptimizerVariablesTableName +
                                 " JOIN " + Tables.OptimizerDefinitions.DefaultCalculatedEconVariablesTableName + " ON " +
                ...
                foreach (string strRxCycle in arrRxCycle)
                {
                    ...
                    p_dataMgr.m_strSQL = "UPDATE " + Tables.OptimizerScenarioResults.DefaultScenarioResultsEconWeightedVariablesRefTableName + " AS r " +
                        "SET " + strFieldName + " = e.weight " +
                        "FROM " + Tables.OptimizerDefinitions.DefaultCalculatedOptimizerVariablesTableName + " AS o JOIN " + Tables.OptimizerDefinitions.DefaultCalculatedEconVariablesTableName + " AS e " +
                        "ON o.ID = e.calculated_variables_id WHERE r.VARIABLE_NAME = o.VARIABLE_NAME AND e.rxcycle = '" + strRxCycle + "'";
